...
function checkKeyRequiredIsNotEmpty(obj) {
    var fieldEmpty = []
    var key_obj = Object.keys(obj)
    for (var i = 0; i < UserFieldRequired.length; i++) {
        if (key_obj.indexOf(UserFieldRequired[i]) > -1 && !obj[UserFieldRequired[i]]) {
            fieldEmpty.push(UserFieldRequired[i])
        }
    }
    return fieldEmpty
}

var middlewareCheckBodyKeysAuthorizedAndRequiredForPut = () => (req, res, next) => {
    var id = req.params.id
    var user_body = req.body
    var user_to_edit = {}
    for (var i = 0; i < UserFieldAuthorized.length; i++) {
        if (_.has(user_body, UserFieldAuthorized[i]))
            user_to_edit[UserFieldAuthorized[i]] = user_body[UserFieldAuthorized[i]]
    }
    var field_required_empty = checkKeyRequiredIsNotEmpty(user_to_edit)
    var user_to_edit_index = _.findIndex(users, ["id", String(id)])
    if (user_to_edit_index > -1 && field_required_empty.length == 0) {
        next()
    }
...
app.put('/user/:id', middlewareCheckBodyKeysAuthorizedAndRequiredForPut,function (req, res) {
...
app.post('/users', middlewareBodyIsArray, function (req, res) {
    var users_to_add = req.body
    var error_element = []
    for (var i = 0; i < users_to_add.length; i++) {
        var user = users_to_add[i]
        var fieldNotAuthorized = checkKeys(user, UserFieldAuthorized)
        var fieldNoRequiredNotMissing = checkObjRequiredKey(user, UserFieldRequired)
